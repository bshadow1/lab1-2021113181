...
public class Graph {
	public static void main(String[] args) {
		...
		while (!exit) {
			// 显示菜单
			System.out.println("1. 查询桥接词（bridge words）");
	...
	private Map<String, Integer> Map;  // 映射
	private int[][] weight;  // 表示图的二维矩阵
	private boolean[][] shortest;  // 表示最短路径的矩阵
	...
	private void Fileprocess(BufferedReader bReader) throws IOException {
		...
		while (bReader.ready()) {
			...
			for (String word : lineArray) {
				if (!word.isEmpty()) {  // 非空单词处理
					...
					Map.putIfAbsent(word, words.size());  // 如果单词未出现过，则添加到Map和words中
		...
		for (int i = 1; i < tempWordList.size(); i++) {
			...
			weight[Map.get(startWord)][Map.get(endWord)]++;  // 权重+1
		}
	}
	...
	public List<String> queryBridgeWords(String startWord, String endWord) {
		...
		// 获取起始单词和结束单词的索引
		int startIndex = Map.get(startWord);
		int endIndex = Map.get(endWord);
	...
	public String  generateNewText(String oldText) {
		...
		// 返回生成的新文本，trim() 方法用于移除字符串的首尾空白字符。
	...
	public void Dijkstra(int v0, int v1, String label) {
		...
		if (dist[v1] != DEFAULT_ARRAY_SIZE) {
			...
			System.out.println("\n");
			String dotFormat = road();
			createGraph(dotFormat, "DGraph" + label);
		} else {
	...
	public void randomWalk(Scanner cin) {
		...
		try {
			...
			// 开始随机游走
			traverse(currentWord, new HashSet<>(), writer, cin, random);
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
			if (writer != null) {
	...
	// 递归函数，实现随机游走
	private void traverse(String currentWord, Set<String> visitedEdges, FileWriter writer, Scanner cin, Random random) throws IOException {
		// 输出当前节点
		...
		neigh.removeAll(visitedEdges);

		// 如果没有未访问的邻接单词，或者用户输入 "s" 则结束游走
		if (neigh.isEmpty() || cin.nextLine().equals("s")) {
			return;
		}
		...
		// 将当前节点到下一个节点的边添加到已经经过的边集合中
		visitedEdges.add(edge);

		// 递归调用继续游走
		traverse(nextWord, visitedEdges, writer, cin, random);
	}
	 ...
	 */
	public static void createGraph(String dotFormat,String fileName)
	{
	...
	public void calcShortestPath(String word1, String word2) {
		...
		// 计算两个单词之间的最短路径，并展示结果
		Dijkstra(Map.get(word1), Map.get(word2), "Calc");
	...
	void calcShortestPath(String word) {
		...
		for (int i = 0; i < words.size(); i++) {
			Dijkstra(Map.get(word), i, word + "To" + words.get(i));
			if (i == rand) {
